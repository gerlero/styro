...
if [ $(command -v styro 2>/dev/null) != "$FOAM_USER_APPBIN/styro" ]; then
...
curl https://github.com/gerlero/styro/releases/latest/download/styro-$(uname)-$(uname -m).tar.gz | tar -xz -C "$FOAM_USER_APPBIN"
